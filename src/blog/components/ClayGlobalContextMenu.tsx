...
import { PageTransitionStyle, TRANSITION_STYLES, getStoredTransitionStyle, saveTransitionStyle } from '../utils/pageTransition';
...
export const ClayGlobalContextMenu: React.FC<ClayGlobalContextMenuProps> = ({
  ...
  // Listen to external transition style changes
  useEffect(() => {
  ...
  useEffect(() => {
    const handleTouchStart = (e: TouchEvent) => {
      ...
      const target = e.target as HTMLElement;
      if (target && (target.tagName === 'INPUT' || target.tagName === 'TEXTAREA' || target.closest('button') || target.closest('a'))) {
        return;
      }
  ...
  const handleActionTransitionCycle = (e: React.MouseEvent) => {
    e.stopPropagation();
    playSoftTick();
    const currIdx = TRANSITION_STYLES.findIndex((s) => s.id === transitionStyle);
    const nextIdx = (currIdx + 1) % TRANSITION_STYLES.length;
    const nextStyle = TRANSITION_STYLES[nextIdx].id;
    setTransitionStyle(nextStyle);
    saveTransitionStyle(nextStyle);
    setIsOpen(false);
  };
  ...
  const currentTransitionObj = TRANSITION_STYLES.find((s) => s.id === transitionStyle) || TRANSITION_STYLES[0];

  const getMenuBadgeTitle = () => {
    if (currentRoute === 'home') return locale === 'zh' ? '🏰 乐园首页工具坞' : 'Home Portal Hub';
    if (currentRoute === 'gallery') return locale === 'zh' ? '📖 旅人笔记工具坞' : 'Notes Space Hub';
    ...
    <>
      ...
      {showInitialHint && typeof document !== 'undefined' && createPortal(
        <div className="fixed bottom-20 left-4 sm:left-8 z-40 animate-in fade-in slide-in-from-bottom-2 duration-300 pointer-events-none select-none">
          <div className="flex items-center gap-2 px-3.5 py-2 rounded-2xl bg-white/95 backdrop-blur-md border border-neutral-200/90 shadow-xl text-neutral-800 text-xs font-cute">
      ...
      {isOpen && typeof document !== 'undefined' && createPortal(
        <motion.div
          ...
          className="fixed z-[250] w-60 bg-[#fdfbf7]/95 backdrop-blur-xl border-3 border-white shadow-2xl rounded-3xl clay-card p-2 text-neutral-800 select-none ring-1 ring-black/5 gpu-layer"
        >
          {/* Header */}
          <div className="flex items-center justify-between px-3 py-1.5 mb-1 border-b border-amber-900/10 text-[11px] font-bubble font-bold text-neutral-500">
              ...
              <>
                ...
                {onRefresh && (
                  <button
                    ...
                    className="w-full flex items-center justify-between px-3 py-2 rounded-2xl hover:bg-emerald-50 hover:text-emerald-700 transition cursor-pointer text-left group"
                  >
                ...
                {/* 2. Theme Cycle (Aligned Icon Box) */}
                <button
                  onClick={handleActionTheme}
                  className="w-full flex items-center justify-between px-3 py-2 rounded-2xl hover:bg-rose-50 hover:text-rose-700 transition cursor-pointer text-left group"
                >
                  <div className="flex items-center gap-2">
                ...
                {/* 3. Transition Animation Cycle */}
                <button
                  onClick={handleActionTransitionCycle}
                  className="w-full flex items-center justify-between px-3 py-2 rounded-2xl hover:bg-purple-50 hover:text-purple-700 transition cursor-pointer text-left group"
                >
                  <div className="flex items-center gap-2">
                    <div className="w-5 h-5 flex items-center justify-center shrink-0">
                      <Wand2 className="w-3.5 h-3.5 text-purple-500" />
                    </div>
                    <span className="font-bold">{locale === 'zh' ? '页面穿梭动效' : 'Transition Style'}</span>
                  </div>
                  <span className="text-[11px] font-bubble font-bold text-purple-600">{currentTransitionObj.emoji} {locale === 'zh' ? currentTransitionObj.nameZh : currentTransitionObj.nameEn}</span>
                </button>
                ...
                <button
                  ...
                  className="w-full flex items-center justify-between px-3 py-1.5 rounded-2xl hover:bg-neutral-100 transition cursor-pointer text-left text-neutral-600"
                >
                  <div className="flex items-center gap-2">
                    <div className="w-5 h-5 flex items-center justify-center shrink-0">
