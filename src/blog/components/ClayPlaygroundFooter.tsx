...
export const ClayPlaygroundFooter: React.FC<ClayPlaygroundFooterProps> = ({
  ...
  return (
    <footer className="mt-16 border-t border-amber-900/10 bg-[#fdfbf7]/90 backdrop-blur-md pt-12 pb-16 px-4 sm:px-8 select-none relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto flex flex-col gap-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div 
            ...
            className="relative p-6 rounded-[32px] bg-white border-3 border-white shadow-md hover:shadow-xl transition cursor-pointer clay-card overflow-hidden group flex flex-col justify-between min-h-[140px]"
          >
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="px-2.5 py-0.5 rounded-full bg-pink-100 text-pink-700 text-xs font-mono font-bold">
                {stampCount} {locale === 'zh' ? '个乐园手印' : 'stamps'}
              </span>
            </div>

            <div>
              <h4 className="font-bubble text-base font-bold text-neutral-900 group-hover:text-rose-600 transition-colors">
          ...
          <div 
            ...
            className="p-6 rounded-[32px] bg-gradient-to-br from-amber-50 to-pink-50 border-3 border-white shadow-md hover:shadow-xl transition cursor-pointer clay-card flex flex-col justify-between min-h-[140px] group"
          >
            <div className="flex items-center justify-between">
              <span className="text-3xl select-none group-hover:rotate-12 transition-transform">🎲</span>
              <span className="px-2.5 py-0.5 rounded-full bg-amber-200/80 text-amber-800 text-xs font-bubble font-bold">
            ...
            <div>
              ...
              <p className="text-xs font-cute text-neutral-500">
          ...
          <div 
            ...
            className="p-6 rounded-[32px] bg-white border-3 border-white shadow-md hover:shadow-xl transition cursor-pointer clay-card flex flex-col justify-between min-h-[140px] group"
          >
            <div className="flex items-center justify-between">
              <span className="text-3xl">📦</span>
              <span className="px-2.5 py-0.5 rounded-full bg-cyan-100 text-cyan-800 text-xs font-mono font-bold">
                JSON Backup
              </span>
            </div>
        ...
        {/* Middle Stats Bar */}
        <div className="p-5 rounded-3xl bg-white/70 border border-neutral-200/80 flex flex-wrap items-center justify-between gap-4 text-xs font-cute text-neutral-600 shadow-3xs">
          <div className="flex items-center gap-4">
            <span className="flex items-center gap-1.5 font-bubble font-bold text-neutral-800">
              <Database className="w-4 h-4 text-rose-500" />
              <span>{totalNotes} {locale === 'zh' ? '篇笔记' : 'notes'}</span>
            </span>
            <span>•</span>
            <span className="font-bold text-pink-600">{totalTags} {locale === 'zh' ? '个标签' : 'tags'}</span>
            <span>•</span>
            <span className="font-bold text-amber-600">{totalWords} {locale === 'zh' ? '字总产出' : 'words'}</span>
          </div>
          ...
          <div className="flex items-center gap-3">
            {onOpenShortcuts && (
              <button
                ...
                className="flex items-center gap-1 hover:text-rose-600 transition cursor-pointer font-bold"
              >
            ...
            <button
              onClick={scrollToTop}
              className="flex items-center gap-1 px-3 py-1 rounded-full bg-neutral-100 hover:bg-neutral-200 text-neutral-700 transition cursor-pointer font-bubble font-bold"
            >
