-- ==============================================================================
-- TAGMESH MARKDOWN: Seed 10 Clean Guest Notes into Cloudflare D1
-- Note: Tags are stored purely in tags_json without cluttering raw_markdown!
-- ==============================================================================

-- Clean existing guest notes if needed
DELETE FROM notes WHERE id LIKE 'guest-seed-%';
DELETE FROM notes_fts WHERE id LIKE 'guest-seed-%';

-- 1. 晨间手冲咖啡
INSERT INTO notes (id, raw_markdown, excerpt, tags_json, word_count, char_count, version, is_pinned, is_deleted, created_at, updated_at, synced_at)
VALUES (
  'guest-seed-01',
  '晨光、浅烘豆与手冲咖啡的治愈早晨 ☕
...
SELECT id, raw_markdown, tags_json FROM notes WHERE id LIKE 'guest-seed-%';
